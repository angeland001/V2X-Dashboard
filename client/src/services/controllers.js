...
/**
 * Fetch audit log entries for a specific controller adapter.
 * @param {string|number} adapterId
 * @param {{ limit?: number }} [opts]
 * @returns {Promise<Array>}
 */
export async function fetchAuditLog(adapterId, { limit = 10 } = {}) {
  const url = new URL(`${API_URL}/api/controllers/${adapterId}/audit-log`);
  url.searchParams.set("limit", String(limit));
  const res  = await fetch(url.toString());
  const data = await res.json();
  if (!res.ok) throw new Error(data.error || `Failed to load audit log (${res.status})`);
