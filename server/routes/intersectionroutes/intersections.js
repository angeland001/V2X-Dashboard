...
router.post("/", async (req, res) => {
  try {
    const { name, description, ref_point, region_id, cuip_slug, created_by } = req.body;
    const normalizedName = normalizeIntersectionName(name);

    if (!normalizedName || !ref_point) {
      return res.status(400).json({ error: "name and ref_point (GeoJSON) are required. intersection_id is auto-generated." });
    }
    ...
    // intersection_id is now auto-generated by the database sequence
    const result = await db.query(
      `INSERT INTO intersections (name, description, ref_point, region_id, cuip_slug, created_by)
       VALUES ($1, $2, ST_SetSRID(ST_GeomFromGeoJSON($3), 4326), $4, $5, $6)
       RETURNING id AS db_id, name, description,
                 ST_AsGeoJSON(ref_point)::json AS ref_point,
                 region_id, intersection_id, msg_issue_revision,
                 status, cuip_slug, created_by, created_at, updated_at`,
      [normalizedName, description || null, JSON.stringify(ref_point), region_id || 0, cuip_slug || null, created_by || null]
    );
